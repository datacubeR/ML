...
pacman::p_load(tidyverse, magrittr, umap, tidymodels, corrplot, fastDummies, patchwork, lubridate)
...
df_train %>%
  ggplot(aes(x = noshow)) +
  geom_histogram(fill = "green", color = "black", binwidth = 3) +
  ggtitle('Distribución de noshow') + 
  theme(plot.title = element_text(hjust = 0.5))
```
```{r}
df_train$nsbin <- as.numeric(df_train$noshow > 3)

p1 <- df_train %>% filter( noshow<30 ) %>%
  ggplot(aes(x = noshow)) +
...
p2 <-df_train %>%
  ...
  ggtitle('Distribución de noshow > 3') + 
  theme(plot.title = element_text(hjust = 0.5))

p1 + p2
summary(df_train$nsbin)
```
...
```{r}
quantile(df_train$noshow, probs = c(0, 0.01,0.05,0.1,0.25,0.5, 0.75, 0.9, 0.95, 0.99, 0.999, 1), na.rm = TRUE)
```
...
```{r}
df_train$wday <- wday(df_train$date, label=TRUE, abbr=FALSE, week_start=1)

w1 <- df_train %>% 
  ggplot(aes(x = wday)) + geom_bar(fill = "blue", color = "black") +
  ggtitle('Distribución wday') + 
  theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
w2 <- ggplot(df_train, aes(x=wday, y=as.numeric(nsbin))) + 
  geom_bar(stat = "identity")+
  ggtitle('Distribución nsbin x wday') + 
  theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
